//-----------------------------------------------------------------------------
//Api_2_Erh_Poab_1.java
//-----------------------------------------------------------------------------

//-----------------------------------------------------------------------------
//Package Definition
//-----------------------------------------------------------------------------
package uicc.test.toolkit.api_2_erh_poab;
...
/**
 * Test Area : uicc.test.toolkit.api_2_erh_poab
 *
 * @version 0.0.1 - 4 avr. 2005
 * @author 3GPP T3 SWG API
 */
public class Api_2_Erh_Poab_1 extends TestToolkitApplet {
    ...
    private static byte[] displayString = {(byte)'A',(byte)'P',(byte)'P',(byte)'L',(byte)'E',(byte)'T',(byte)'1'};
 
    private static final byte DCS_8_BIT_DATA = 0x04;
    /**
    * Constructor of the applet
    */
    public Api_2_Erh_Poab_1() 
    {
    }
    ...
    */
    public static void install(byte bArray[], short bOffset, byte bLength) 
    {
        ...
        Api_2_Erh_Poab_1  thisApplet = new Api_2_Erh_Poab_1();
        ...
        // Register on EVENT DOWNLOAD CALL CONTROL BY NAA
    ...
    public void processToolkit(short event) 
    {
        ...
        case (byte) 0 :
                // --------------------------------------------
                // Test Case 1 : getTheHandler and than post(true)
                testCaseNb = (byte) 1 ;
        ...
        case (byte) 1 :
                // --------------------------------------------
                // Test Case 2 : Append data and post(true)
                testCaseNb = (byte) 2 ;
                
                bRes = false ;
                try {
                    envRespHdlr.appendTLV((byte)0x01,data,(short) 0x00, (short)0x00FA);
                    envRespHdlr.postAsBERTLV(true,(byte)0x33);
                    bRes = true;
                }
                catch (Exception e)    {    
                    bRes = false ;
        ...
        case (byte) 2 :
                // --------------------------------------------
                // Test Case 3 : use the methode appendTLV after the using of the post(true) methode 
                testCaseNb = (byte) 3 ;
                
                bRes = false ;
                try {
                    envRespHdlr.appendTLV((byte)0x01,data,(short) 0x00, (short) 0x0010);
                    envRespHdlr.postAsBERTLV(true,(byte)0x33);
        ...
        case (byte) 3 :
                // --------------------------------------------
                // Test Case 4 : use the methode post(FALSE) and send a display text
                testCaseNb = (byte) 4 ;
        
                bRes = false ;
                try {
                    envRespHdlr.appendTLV((byte)0x01,data,(short) 0x00, (short) 0x0010);
                    envRespHdlr.postAsBERTLV(false,(byte)0x75);
        ...
        case (byte) 4 :
                // --------------------------------------------
                // Test Case 5: use the methode postAsBERTLV() without appendTLV() and send a display text
                testCaseNb = (byte) 5 ;
                
                bRes = false ;
                try {
                    
                    envRespHdlr.postAsBERTLV(false,(byte)0x33);
        ...
        case (byte) 5 :
                // --------------------------------------------
                // Test Case 6 : use the postAsBERTLV() methode after a display text command
                testCaseNb = (byte) 6 ;
                
                bRes = false ;
                try{
                   envRespHdlr.appendTLV((byte)0x01,data,(short) 0x10, (short) 0x0010);
                   //send the proactive command
                   proHdlr.initDisplayText((byte)0x80, DCS_8_BIT_DATA, displayString, (short) 0, (short) displayString.length);
                   proHdlr.send();
        ...
        case (byte) 6 :
                // --------------------------------------------
                // Test Case 7 : try to use the methode postAsBERTLV(true) after the using of the methode postAsBERTLV(false) 
                testCaseNb = (byte) 7 ;
                
                bRes = false ;
                try {
                    envRespHdlr.appendTLV((byte)0x01,data,(short) 0x00, (short) 0x0010);
                    envRespHdlr.postAsBERTLV(false,(byte)0x56);
                    bRes = false;
                    try{
                        envRespHdlr.postAsBERTLV(true,(byte)0x28);
                    }
                    catch (ToolkitException e){
                        if(e.getReason()==ToolkitException.HANDLER_NOT_AVAILABLE)
                            bRes = true ;
        ...
        case (byte) 7 :
            // --------------------------------------------
            // Test Case 8 : In case of CALL_CONTROL the CAT Runtime Environment set the boolean value always to true 
            testCaseNb = (byte) 8 ;
        ...
        case (byte) 8 :
            // --------------------------------------------
            // Test Case 9 : In case of CALL_CONTROL the CAT Runtime Environment set the boolean value always to true 
            testCaseNb = (byte) 9 ;
            
            bRes = false ;
            try {
                envRespHdlr.appendTLV((byte)0x01,data,(short)0x0010,(short)0x0010);
                envRespHdlr.postAsBERTLV(true,(byte)0x28);
                bRes = true;
            }
            catch (Exception e){
        ...
        case (byte) 9 :
            // --------------------------------------------
            // Test Case 10 : Resulting response length greater than 256. 
            testCaseNb = (byte) 10 ;
            
            bRes = false ;
            try {
                envRespHdlr.appendArray(data,(short) 0x00,(short)0x00FE);
                bRes = false;
                try{
                    envRespHdlr.postAsBERTLV(true,(byte)0x33);
                }
                catch (APDUException e){
                    if(e.getReason()==APDUException.BAD_LENGTH)
                        bRes = true ;
                }
            }
            catch (Exception e){
